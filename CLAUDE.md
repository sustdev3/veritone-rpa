...
| node-cron 3.0 | Installed — scheduler not yet implemented |
...
| `SMTP_HOST` | `smtp.gmail.com` | SMTP server for fault notifications |
| `SMTP_PORT` | `587` | SMTP port |
| `SMTP_USER` | — | Sending email address |
| `SMTP_PASS` | — | Gmail app password |
| `NOTIFY_EMAIL_1` | `bruce@8020green.com` | Fault notification recipient 1 |
| `NOTIFY_EMAIL_2` | `anthony@8020green.com` | Fault notification recipient 2 |
| `RUN_MODE` | `testing` | `production` enforces 7 pm–7 am window; `testing` skips it |
| `LOOKBACK_DAYS` | `10` | How many days back to look for adverts |

**Currently:** `RUN_MODE=testing` is active. Switch to `production` before going live.
...
3. `filterAndSort()` in `advert-page-object.ts` slices the first 5 adverts from the lookback
   window rather than processing all of them.
...
| `src/adverts/advert-page-object.ts` (`filterAndSort`) | The log line and `return filtered.slice(0, 5)` — replace with `return filtered` |
...
- **Scheduler** — implement the 7 pm–7 am Sydney time window using `node-cron`
...
- `node-cron` is listed as a dependency but no scheduler code exists yet
- Remove all `// TESTING ONLY` overrides before switching to `RUN_MODE=production` (see §8)
